...
**В продукте найдено тринадцать дефектов**, у каждого есть воспроизведение, которое отрабатывает за секунды и ничего не импортирует из фреймворка. Подробности в [docs/findings](docs/findings/).

**AI-first инженерия.** Фреймворк построен так, как я работаю: с ИИ в контуре. ИИ ускоряет реализацию и расширяет поиск дефектов, а архитектура, стратегия тестирования и каждый компромисс остаются инженерными решениями, и каждое записано в [docs/strategy.ru.md](docs/strategy.ru.md) вместе с альтернативами, от которых я отказался в его пользу. Именно это сочетание позволяет одному инженеру дать такую глубину покрытия: большая часть набора порождается из описаний API самого продукта, и он уже нашёл тринадцать настоящих дефектов.

## Статус

| | |
|---|---|
| Основной прогон | 1016 пройдено, 16 пропущено, 3 ожидаемых падения |
| Порождено из описаний самого продукта | 744 из 830 тестов, привязанных к матрице |
| Время прогона на восьми исполнителях | около 47 секунд |
| Задействовано операций API | 99% обеих версий |
| Собственные тесты фреймворка, стенд не нужен | 177 за 12 секунд |
| Слой устойчивости, отдельным прогоном | 6 |
| Найдено дефектов продукта | 13 |

Две строки [матрицы покрытия](docs/coverage-matrix.ru.md) до сих пор пусты, и прогон сам говорит об этом в конце каждого запуска: у бизнес-правил (`FUN`) и конкурентности (`CNC`) тестов пока нет. Всё остальное покрыто на ту глубину, которую описывает матрица.
...
**Фиксированных пауз нет ни одной.** Ожидание опрашивает условие с явным сроком, и по истечении срока сообщение говорит, чего именно оно ждало. Единственное исключение названо в том тесте, который его держит, и модульный тест роняет сборку на любом другом.
...
Тринадцать дефектов, каждый в своей папке с описанием на двух языках и воспроизведением:
