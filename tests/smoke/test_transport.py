"""Proves the transport and identity layers work.

These are not product tests. They exist so that a failure anywhere else
in the suite can be trusted to be about the product rather than about the
...
def test_two_actors_are_isolated(actors: ActorFactory) -> None:
    ...
    assert all(p["id"] != project_id for p in visible_to_bob.json), (
        "bob can see a project he has no relationship to; ownership-based "
