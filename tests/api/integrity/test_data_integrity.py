...
The bulk case is the point of this file. Checking permissions on the first
element of a list and then applying the whole list is a classic defect, and
it is invisible from the API side, because the refusal looks identical
whether or not something was written before it.

Every query here selects rows by the identifiers this test created, never
by a title or other value. Eight workers run these at once, so a query by
title can return another worker's row, compare it against this test's
data, and pass without having checked anything.
"""
